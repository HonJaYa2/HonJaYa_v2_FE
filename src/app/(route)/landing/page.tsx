...
const Landing: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchPreferences = async () => {
          try {
              const response = await axios.get(`/api/getPreferencesStatus/${cookies.user.id}`);
              return response.data.preferences_completed;
          } catch (error) {
              console.error('Failed to fetch preferences status:', error);
              return null;
          }
      };
            ...
            <div className="w-full h-auto">
                {features.map((contents, index) => (
                    <FeatureContainer
                        ...
                        imageUrl={contents[1][0] as string}
